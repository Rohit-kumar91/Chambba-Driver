...
class YourTripsVC: UIViewController {
    ...
    func onGetHistroy(apiName: String) {
        ...
        ServiceHelper.sharedInstance.createGetRequest(isShowHud: true, params: [:], apiName: apiName) { (response, error) in
            ...
            if response != nil {
                ...
                self.result = JSON(response as Any).arrayValue
                
                print("Array", self.result)
    ...
    func cancelScheduleRide(reqID: String) {
        ...
        ServiceHelper.sharedInstance.createPostRequest(isShowHud: true, params: param as [String : AnyObject], apiName: API_REQUEST_CANCEL) { (response, error) in
